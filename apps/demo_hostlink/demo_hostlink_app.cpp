...
aegis::sdk::AppRunResult run_demo_hostlink_abi(const aegis_host_api_v1_t* host) {
    const aegis::sdk::HostApiClient api(host);
    if (!api.valid()) {
        return aegis::sdk::AppRunResult::Failed;
    }

    api.log_write("demo_hostlink", "entered hostlink demo app");
    api.notification().publish("Hostlink Demo", "Runtime admitted hostlink transport usage");
...
extern "C" aegis_app_run_result_v1_t demo_hostlink_main(const aegis_host_api_v1_t* host_api) {
    return aegis::sdk::to_abi_result(run_demo_hostlink_abi(host_api));
}
