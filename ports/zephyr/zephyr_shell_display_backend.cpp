...
int PagerDisplayBackend::raw_set_cursor(uint16_t x, uint16_t y, uint16_t width, uint16_t height) const {
    ...
    int rc = raw_send_command_unlocked(kSt7796sCmdColumnAddressSet, reinterpret_cast<const uint8_t*>(address_data), sizeof(address_data));
    if (rc != 0) {
        return rc;
    }
    address_data[0] = sys_cpu_to_be16(y0);
    address_data[1] = sys_cpu_to_be16(static_cast<uint16_t>(y0 + height - 1));
    return raw_send_command_unlocked(kSt7796sCmdRowAddressSet, reinterpret_cast<const uint8_t*>(address_data), sizeof(address_data));
}
...
int PagerDisplayBackend::raw_write_pixels_unlocked(int x, int y, int width, int height, const std::vector<uint16_t>& pixels) const {
    ...
    auto raw_config = make_raw_mipi_config();
    return mipi_dbi_write_display(raw_mipi_device_, &raw_config, reinterpret_cast<const uint8_t*>(pixels.data()), &desc, PIXEL_FORMAT_RGB_565);
}
