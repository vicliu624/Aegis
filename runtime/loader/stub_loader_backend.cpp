...
StubLoaderBackend::StubLoaderBackend(platform::Logger& logger)
    : StubLoaderBackend(logger, std::make_unique<StubLlextAdapter>(logger)) {}

StubLoaderBackend::StubLoaderBackend(platform::Logger& logger, LlextAdapterPtr adapter)
    : logger_(logger), adapter_(std::move(adapter)) {}

LoaderPreparationResult StubLoaderBackend::prepare(const core::AppSession& session) {
...
bool StubLoaderBackend::can_resolve_entry_symbol(const std::string& entry_symbol) const {
    return can_resolve_compiled_entry_symbol(entry_symbol);
}

LoaderResult StubLoaderBackend::load(core::AppSession& session) {
...
LoaderResult StubLoaderBackend::bringup(core::AppSession& session, HostApi& host_api) {
...
LoaderResult StubLoaderBackend::teardown(core::AppSession& session,
...
LoaderResult StubLoaderBackend::unload(core::AppSession& session) {
...
sdk::AppRuntimeContract StubLoaderBackend::resolve_contract(const std::string& entry_symbol) {
